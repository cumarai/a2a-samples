...
class OrderManagementAgentExecutor(AgentExecutor):
    """Order Management AgentExecutor Example."""

    def __init__(self):
        self.agent = OrderManagementAgent()
    ...
    async def execute(
        ...
        async for item in self.agent.stream(query, task.contextId):
            ...
            if not is_task_complete:
                ...
                continue
            # If the response is a dictionary, handle our order management response format
            if isinstance(item['content'], dict):
                # Check if it's our order management response format
                if 'message' in item['content']:
                    ...
                    # If there are orders, add them as a data artifact
                    if 'orders_list' in item['content'] and item['content']['orders_list']:
                        orders_data = {
                            'orders': item['content']['orders_list']
                        }
                        await updater.add_artifact(
                            [Part(root=DataPart(data=orders_data))], 
                            name='orders'
                        )
                    continue
                else:
                ...
                await updater.add_artifact(
                    [Part(root=TextPart(text=item['content']))], name='order'
                )
